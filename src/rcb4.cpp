...
//int kondo_set_angle(KondoRef ki, UINT jointIndex, double angleInDegree, double fractionMaxSpeed)
//{
...
////int kondo_set_angles(KondoRef ki, std::vector<int> jointIndices, double *anglesInDegree, double fractionMaxSpeed)
////{
////    assert(ki);
////    int ret;
...
////    int jointNumbers =
////    ki->swap[2] = 0b00111101;
////    ki->swap[3] = 0b11110011;
////    ki->swap[4] = 0b01111110;
////    ki->swap[5] = 0b00000000;
////    ki->swap[6] = 0b00000000;

////    // servo speed
////    ki->swap[7] = round(127*(1-fractionMaxSpeed)); // velocity
////    printf("speed: %i\n",ki->swap[7]);
...
////        for (; temp[i]; c++)
////            temp[i] &= temp[i] - 1;
////    printf("number of servos to move: %i\n",c);

////    // for each servo prepare angle to move
////    for (i = 8; i < 8 + (c * 2); i += 2) {
////        printf("angle for joint %i: %f\n",i,anglesInDegree[(i-8)/2]);
////        ki->swap[i]     = (int)round((anglesInDegree[(i-8)/2]*29)+7500)%256; // position L
////        printf("position L: %x\n",(int)round((anglesInDegree[(i-8)/2]*29)+7500)%256);
